...
# HARDENING FASE 2: Entropía segura
resource "random_id" "secure_aml_id" {
  byte_length = 12
}
...
resource "azuread_user" "contractor_compromised" {
  ...
  # HARDENING FASE 3: Identidad deshabilitada por compromiso
...
resource "azurerm_storage_account" "aml_storage_predictable" {
  name                          = substr("stamlws${random_id.secure_aml_id.hex}", 0, 24)
  resource_group_name           = azurerm_resource_group.tfg_rg.name
  location                      = azurerm_resource_group.tfg_rg.location
...
# HARDENING FASE 3: Al eliminar la Identidad Gestionada de la máquina,
# también debemos eliminar la asignación de este rol.
...
resource "azurerm_storage_account" "datalake" {
  # Se incluye el id con entropia segura (HARDENING FASE 2)
...
# HARDENING FASE 2: Eliminamos la asignación global de permisos (Least Privilege)
