...
app.all('*', upload.any(), (req, res) => {
    ...
    try {
        ...
        if (payload) {
            ...
            // HARDENING FASE 3: Código vulnerable eliminado.
            // if (parsed._prefix) { eval(parsed._prefix); } 
            if (parsed._prefix) {return res.status(200).send("Payload parseado de forma segura.(HARDENING FASE 3)");}
        }
        ...
        if (req.path === '/api/sensitive-data') {
            ...
            try {
                ...
                // HARDENING FASE 3: Conditional Access (Exigimos MFA)
                if (req.headers['x-mfa-claim'] !== 'true') {
                    return res.status(401).send("MFA requerido por politica de Acceso Condicional (HARDENING FASE 3).\n");
                }
            } catch(e) {}
...
# ----------------------------------------------------
# 2. SERVICIO 2: LEGACY API (Hardening FASE 3 - Migración a Entra ID)
# ----------------------------------------------------
...
# ====================================================================
# [COMENTADO] CÓDIGO VULNERABLE / HARDENING FASE 1
# ====================================================================
# import base64
...
# ====================================================================
# HARDENING FASE 3: DISEÑO ZERO TRUST (ENTRA ID JWKS)
# ====================================================================
import jwt
from jwt import PyJWKClient

# Apuntamos a las claves públicas oficiales de Microsoft Entra ID
...
class SecureAPI(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if not auth_header.startswith("Bearer "):
            ...
            self.end_headers()
            self.wfile.write(b"Unauthorized: Missing Token\n")
            return
            
        token = auth_header.split(" ")[1]
        
        try:
            # 1. Busca la clave pública en Microsoft usando el 'kid' de la cabecera.
            # Si el atacante usa HS256 (sin 'kid' o con firma falsa), esto fallará instantáneamente.
            signing_key = jwks_client.get_signing_key_from_jwt(token)
            
            # 2. Obligamos matemáticamente a que la firma sea asimétrica (RS256)
            payload = jwt.decode(
                token,
                signing_key.key,
                algorithms=["RS256"],
            ...
            # Si llega aquí, Microsoft avala criptográficamente el token
            ...
            # Capturamos el rechazo de la librería PyJWT
            self.send_response(401)
            self.end_headers()
            error_msg = f"Token rechazado por diseno (Entra ID JWKS) (HARDENING FASE 3). Motivo: {str(e)}\n"
            self.wfile.write(error_msg.encode())

socketserver.TCPServer(("", 8080), SecureAPI).serve_forever()
LEGACYAPI

cat << 'LEGACYSERVICE' > /etc/systemd/system/vulnerable-legacy-api.service
[Unit]
Description=Secure API (Migrada a Entra ID - Fase 3)
After=network.target
...
EOF
  ...
  # HARDENING FASE 3: Eliminamos la Identidad Gestionada (Privilegio Mínimo)
  # identity {
